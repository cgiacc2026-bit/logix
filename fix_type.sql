...
-- ==============================================================================
-- 1) التحقق من الحقول الإضافية وإصلاح قيود (Not-Null)
-- ==============================================================================
...
-- في حالة تمت إضافة الحقول بشكل خاطئ كـ UUID في المحاولة السابقة، نقوم بتعديلها إلى TEXT
ALTER TABLE public.journal_entries ALTER COLUMN debit_account TYPE TEXT USING debit_account::text;
ALTER TABLE public.journal_entries ALTER COLUMN credit_account TYPE TEXT USING credit_account::text;

-- لتجنب أي أخطاء متعلقة بالقيود الصارمة (Not-Null) إذا كانت مفروضة مسبقاً في هيكل قاعدة البيانات
...
BEGIN
    ...
    IF v_voucher.type = 'RECEIPT' THEN
        ...
        v_debit_account := v_liquid_acc.id::text;
        v_credit_account := v_receivable_acc.id::text;
    ...
    ELSE
        ...
        v_debit_account := v_payable_acc.id::text;
        v_credit_account := v_liquid_acc.id::text;
