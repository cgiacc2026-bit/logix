...
export const AccountStatementView: React.FC<AccountStatementViewProps> = ({
  ...
  return (
    <div className="space-y-6 dir-rtl text-right">
      ...
      {statementResult ? (
        <div
          className={`bg-white border border-[#E2E8F0] rounded-2xl shadow-sm overflow-hidden printable-card transition-transform duration-150 origin-top print-scalable-container ${getFontScaleClass()}`}
          style={{
        ...
        >
          ...
          <div className="p-6 md:p-8 bg-[#FDFCFB] border-b border-[#E2E8F0] space-y-6">
            <div className="flex flex-col md:flex-row items-stretch justify-between gap-6 pb-6 border-b border-[#E2E8F0]">
              {/* الطرف الأيمن: إطار الشعار المؤسسي وبيانات الشركة */}
              <div className="flex items-center gap-4.5 flex-1">
                <div className="w-16 h-16 md:w-20 md:h-20 rounded-2xl bg-gradient-to-br from-[#1A1A1A] via-[#2A2A2A] to-[#1A1A1A] text-white p-0.5 shadow-md flex flex-col items-center justify-center shrink-0 border-2 border-[#D4AF37]/50 relative overflow-hidden">
                  ...
                  <Building2 className="w-7 h-7 md:w-8 md:h-8 text-[#D4AF37] relative z-10 mb-0.5" />
                  <span className="text-[9px] md:text-[10px] font-serif font-black tracking-widest text-[#D4AF37] uppercase relative z-10">
                    AL-WALEED
                  </span>
                </div>
                ...
                <div className="space-y-1">
                  <div className="flex items-center gap-2">
                    <h1 className="text-xl md:text-2xl font-serif font-extrabold text-[#1A1A1A] tracking-tight">
                      {company?.nameAr || 'مطحنة الوليد المتحدة'}
                    </h1>
                    <span className="px-2 py-0.5 bg-[#FAF3E0] text-[#996515] border border-[#D4AF37]/30 text-[10px] font-bold rounded-full hidden sm:inline-flex items-center gap-1">
                  ...
                  <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-[#475569] pt-1 font-sans">
                    {company?.commercialRegNumber && (
                      <span className="bg-[#F1F5F9] px-2 py-0.5 rounded border border-[#E2E8F0]">
                        سجل تجاري: <strong className="font-mono text-[#0F172A]">{company.commercialRegNumber}</strong>
                      </span>
                    )}
                    {company?.taxNumber && (
                      <span className="bg-[#F1F5F9] px-2 py-0.5 rounded border border-[#E2E8F0]">
                        الرقم الضريبي: <strong className="font-mono text-[#0F172A]">{company.taxNumber}</strong>
                      </span>
                    )}
                    {company?.phone && (
                      <span>
                        هاتف: <strong className="font-mono text-[#0F172A]">{company.phone}</strong>
                      </span>
                    )}
                  </div>
                </div>
              ...
              <div className="flex flex-col justify-between bg-[#F8FAFC] p-4 rounded-xl border border-[#E2E8F0] shrink-0 min-w-[240px] text-right shadow-2xs">
                <div className="flex items-center justify-between border-b border-[#E2E8F0] pb-2 mb-2">
                  <span className="text-[11px] font-serif font-black text-[#B8860B] uppercase tracking-wider flex items-center gap-1">
                    <ShieldCheck className="w-3.5 h-3.5 text-[#B8860B]" />
                    سند كشف حساب معتمد
                  </span>
                  <span className="text-[10px] font-mono text-[#64748B] bg-white px-1.5 py-0.5 rounded border border-[#E2E8F0]">
                    ORIGINAL
                  </span>
                </div>
                ...
                <div className="space-y-1.5 text-xs sm:text-sm">
                  <div className="flex items-center justify-between">
                    <span className="text-[#64748B] font-medium">كود الحساب:</span>
                    <span className="font-mono font-bold text-[#0F172A] bg-white px-2 py-0.5 rounded border border-[#E2E8F0] text-xs">
                      {statementResult.entityCode}
                    </span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-[#64748B] font-medium">تاريخ الطباعة:</span>
            ...
            <div className="text-center space-y-2 py-2">
              <h2 className="text-xl md:text-2xl font-serif font-black text-[#0F172A] tracking-tight">
                كشف حساب {statementResult.entityType === 'CUSTOMER' ? 'العميل' : 'المورد'}:{' '}
                <span className="text-[#1E293B] border-b-2 border-[#D4AF37] pb-0.5">
                  {statementResult.entityNameAr}
                </span>
              </h2>
              <p className="text-xs sm:text-sm font-semibold text-[#64748B] flex items-center justify-center gap-2">
                <span>عن الفترة المالية من:</span>
                <span className="font-mono font-bold text-[#0F172A] px-2 py-0.5 bg-[#F1F5F9] border border-[#CBD5E1] rounded">
                  {statementResult.startDate}
                </span>
                <span>إلى:</span>
                <span className="font-mono font-bold text-[#0F172A] px-2 py-0.5 bg-[#F1F5F9] border border-[#CBD5E1] rounded">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 bg-[#F8FAFC] p-4.5 rounded-xl border border-[#E2E8F0] text-xs sm:text-sm">
              <div className="space-y-1">
                <span className="text-[#64748B] block font-medium">اسم الجهة / الحساب التجاري:</span>
                <span className="font-bold text-[#0F172A] text-sm sm:text-base">{statementResult.entityNameAr}</span>
              </div>
              <div className="space-y-1 sm:border-r sm:border-l sm:border-[#E2E8F0] sm:px-4">
                <span className="text-[#64748B] block font-medium">الرقم الضريبي / السجل التجاري:</span>
                <span className="font-mono font-bold text-[#0F172A] text-sm">{statementResult.entityTaxNo || 'غير مسجل'}</span>
              </div>
              <div className="space-y-1">
                <span className="text-[#64748B] block font-medium">حد الائتمان / رقم التواصل:</span>
          ...
          {/* جدول حركات الحساب بمعايير المحاسبة وتوسيع عمود البيان */}
          <div className="overflow-x-auto print:overflow-visible">
            <table className="w-full text-right text-xs sm:text-sm border-collapse">
              <thead>
                <tr className="bg-[#F1F5F9] text-[#0F172A] border-b border-[#CBD5E1] font-serif font-bold text-xs sm:text-sm">
                  <th className="py-3.5 px-3 border-r border-[#E2E8F0] text-center w-12 shrink-0">#</th>
                  <th className="py-3.5 px-3 border-r border-[#E2E8F0] w-28 whitespace-nowrap">التاريخ</th>
                  <th className="py-3.5 px-3 border-r border-[#E2E8F0] w-32 whitespace-nowrap">رقم المستند</th>
                  <th className="py-3.5 px-3 border-r border-[#E2E8F0] w-36 whitespace-nowrap">نوع الحركة</th>
                  <th className="py-3.5 px-4 border-r border-[#E2E8F0] min-w-[280px]">البيان والشرح المحاسبي</th>
                  <th className="py-3.5 px-3 border-r border-[#E2E8F0] text-left w-32 text-[#15803D] whitespace-nowrap">
                    مدين (+)
                  </th>
                  <th className="py-3.5 px-3 border-r border-[#E2E8F0] text-left w-32 text-[#B91C1C] whitespace-nowrap">
                    دائن (-)
                  </th>
                  <th className="py-3.5 px-3 border-r border-[#E2E8F0] text-left w-36 font-extrabold text-[#0F172A] whitespace-nowrap">
                    الرصيد الجاري
                  </th>
              ...
              <tbody className="divide-y divide-[#E2E8F0]">
                ...
                <tr className="bg-[#FBF9F4] font-bold text-[#0F172A] border-b border-[#E2E8F0]">
                  <td className="py-3.5 px-3 text-center border-r border-[#E2E8F0] text-[#B8860B] font-mono">0</td>
                  <td className="py-3.5 px-3 border-r border-[#E2E8F0] font-mono text-[#64748B] whitespace-nowrap">
                    {statementResult.startDate}
                  </td>
                  <td className="py-3.5 px-3 border-r border-[#E2E8F0] font-mono text-[#64748B] whitespace-nowrap">
                    OPEN-BAL
                  </td>
                  <td className="py-3.5 px-3 border-r border-[#E2E8F0] whitespace-nowrap">
                    <span className="inline-flex items-center gap-1.5 text-[#B8860B] font-bold">
                      <span className="text-[#B8860B] text-sm leading-none font-sans font-black">▸</span>
                      <Clock className="w-3.5 h-3.5" />
                      <span>رصيد افتتاحي</span>
                    </span>
                  </td>
                  <td className="py-3.5 px-4 border-r border-[#E2E8F0] text-[#475569] font-medium whitespace-nowrap">
                    الرصيد الافتتاحي المنقول من قبل تاريخ {statementResult.startDate}
                  </td>
                  <td className="py-3.5 px-3 border-r border-[#E2E8F0] text-left font-mono font-bold text-[#15803D] whitespace-nowrap">
                    {statementResult.openingBalance > 0 && statementResult.openingBalanceType === 'DEBIT'
                      ? statementResult.openingBalance.toFixed(3)
                      : '-'}
                  </td>
                  <td className="py-3.5 px-3 border-r border-[#E2E8F0] text-left font-mono font-bold text-[#B91C1C] whitespace-nowrap">
                    {statementResult.openingBalance > 0 && statementResult.openingBalanceType === 'CREDIT'
                      ? statementResult.openingBalance.toFixed(3)
                      : '-'}
                  </td>
                  <td className="py-3.5 px-3 border-r border-[#E2E8F0] text-left font-mono text-sm sm:text-base font-black text-[#0F172A] whitespace-nowrap">
                    {formatCurrency(statementResult.openingBalance, currency)}
                  </td>
                ...
                {filteredTransactions.length > 0 ? (
                    ...
                    return (
                      <tr
                        ...
                        className={`hover:bg-[#F8FAFC] transition-colors ${
                          isCancelled
                            ? 'bg-rose-50/40 text-slate-400 line-through'
                            : isReversal
                            ? 'bg-amber-50/40'
                            : ''
                        }`}
                      >
                        <td className="py-3 px-3 text-center border-r border-[#E2E8F0] font-mono text-[#64748B]">
                          {idx + 1}
                        </td>
                        <td className="py-3 px-3 border-r border-[#E2E8F0] font-mono text-[#1E293B] whitespace-nowrap">
                          {tx.date}
                        </td>
                        <td className="py-3 px-3 border-r border-[#E2E8F0] font-mono font-bold text-[#0F172A] whitespace-nowrap">
                          {tx.docNumber}
                        </td>
                        <td className="py-3 px-3 border-r border-[#E2E8F0] font-bold text-[#1E293B] whitespace-nowrap">
                          {tx.docTypeLabel}
                        </td>
                        <td className="py-3 px-4 border-r border-[#E2E8F0] text-[#334155] leading-relaxed">
                          {tx.description}
                        </td>
                        <td className="py-3 px-3 border-r border-[#E2E8F0] text-left font-mono font-bold text-[#15803D] whitespace-nowrap">
                          {tx.debit > 0 ? tx.debit.toFixed(3) : '-'}
                        </td>
                        <td className="py-3 px-3 border-r border-[#E2E8F0] text-left font-mono font-bold text-[#B91C1C] whitespace-nowrap">
                          {tx.credit > 0 ? tx.credit.toFixed(3) : '-'}
                        </td>
                        <td className="py-3 px-3 border-r border-[#E2E8F0] text-left font-mono font-black text-[#0F172A] text-xs sm:text-sm whitespace-nowrap">
                  ...
                  <tr>
                    <td colSpan={9} className="p-4 sm:p-6 bg-[#FAFAFA]">
                      <div className="max-w-xl mx-auto my-2 bg-amber-50/50 border border-amber-200/70 rounded-xl p-5 text-center shadow-2xs space-y-2.5">
                        <div className="w-10 h-10 rounded-full bg-amber-100/80 border border-amber-300/60 flex items-center justify-center mx-auto text-amber-700">
                ...
                <tr className="bg-[#1E293B] text-white font-serif font-black text-xs sm:text-sm border-t-2 border-[#0F172A]">
                  <td
                    colSpan={5}
                    className="py-4 px-4 border-r border-slate-700 text-left font-bold text-slate-100 tracking-wide"
                  >
                    <div className="flex items-center justify-between">
                      <span className="text-[10px] text-amber-300 font-mono font-normal">
                        CLOSING BALANCE & TOTALS
                      </span>
                      <span className="text-right">إجمالي حركات الفترة والرصيد الختامي المرحل:</span>
                    </div>
                  </td>

                  {/* مجموع المدين تحت عمود مدين */}
                  <td className="py-4 px-3 border-r border-slate-700 text-left font-mono text-sm font-bold text-emerald-400 whitespace-nowrap bg-slate-800/40">
                    {statementResult.totalPeriodDebit.toFixed(3)}
                  </td>

                  {/* مجموع الدائن تحت عمود دائن */}
                  <td className="py-4 px-3 border-r border-slate-700 text-left font-mono text-sm font-bold text-rose-400 whitespace-nowrap bg-slate-800/40">
                    {statementResult.totalPeriodCredit.toFixed(3)}
                  </td>

                  {/* الرصيد الختامي تحت عمود الرصيد الجاري */}
                  <td className="py-4 px-3 border-r border-slate-700 text-left font-mono text-sm sm:text-base font-black text-amber-300 whitespace-nowrap bg-slate-900/60">
                    {formatCurrency(statementResult.closingBalance, currency)}
                  </td>
          ...
          <div className="p-6 md:p-10 bg-[#FDFCFB] border-t border-[#E2E8F0] space-y-8 pb-12">
            {/* نص إقرار وصحة الرصيد المحاسبي */}
            <div className="text-xs sm:text-sm text-[#475569] leading-relaxed bg-[#F8FAFC] p-4 rounded-xl border border-[#E2E8F0]">
              <div className="flex items-start gap-2">
                <CheckCircle2 className="w-4 h-4 text-[#15803D] shrink-0 mt-0.5" />
                <div>
                  <strong className="text-[#0F172A] font-bold">إقرار وصحة الرصيد المالي:</strong> يُعتبر هذا الكشف صحيحاً
            ...
            <div className="pt-4 grid grid-cols-1 sm:grid-cols-3 gap-8 md:gap-12 text-center text-xs sm:text-sm">
              <div className="space-y-3 p-4 bg-white rounded-xl border border-[#E2E8F0] shadow-2xs">
                <span className="text-[#0F172A] font-bold block">إعداد المحاسب المسؤول</span>
                <span className="text-[#64748B] text-xs block">التوقيع والتاريخ:</span>
                <div className="w-full pt-6">
                  <div className="w-4/5 h-[1.5px] bg-[#94A3B8] mx-auto border-dashed border-t border-[#64748B]"></div>
                  <span className="text-[10px] text-[#94A3B8] font-mono block pt-1">Signature & Date</span>
                </div>
              </div>

              <div className="space-y-3 p-4 bg-white rounded-xl border border-[#E2E8F0] shadow-2xs">
                <span className="text-[#0F172A] font-bold block">مراجعة وتدقيق الحسابات</span>
                <span className="text-[#64748B] text-xs block">التوقيع والتاريخ:</span>
                <div className="w-full pt-6">
                  <div className="w-4/5 h-[1.5px] bg-[#94A3B8] mx-auto border-dashed border-t border-[#64748B]"></div>
                  <span className="text-[10px] text-[#94A3B8] font-mono block pt-1">Auditor Review</span>
                </div>
              </div>

              <div className="space-y-3 p-4 bg-white rounded-xl border border-[#E2E8F0] shadow-2xs">
                <span className="text-[#0F172A] font-bold block">اعتماد المدير المالي / الختم</span>
                <span className="text-[#64748B] text-xs block">الختم الرسمي والتوقيع:</span>
                <div className="w-full pt-6">
                  <div className="w-4/5 h-[1.5px] bg-[#94A3B8] mx-auto border-dashed border-t border-[#64748B]"></div>
                  <span className="text-[10px] text-[#94A3B8] font-mono block pt-1">Financial Officer & Stamp</span>
                </div>
              </div>
            </div>
