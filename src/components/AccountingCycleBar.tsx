...
export const AccountingCycleBar: React.FC<AccountingCycleBarProps> = ({
  ...
  const steps: CycleStep[] = [
    ...
    },
    {
      ...
      sub: 'أوامر التشغيل والخطوط',
      icon: Factory,
      targetTab: 'production',
      badge: 'تصنيع شامل',
      description: 'إصدار وتوجيه أوامر التشغيل الصناعي، واستهلاك المواد الخام والمكونات، واحتساب الهالك والمنتجات العرضية، وتكلفة الوحدة التامة.',
      inputs: 'قائمة المواد (BOM)، تكاليف التشغيل المباشرة، خطوط الإنتاج والمحطات، نسب الهالك.',
      outputs: 'إضافة فورية للمنتجات التامة بالمستودع، خصم مكونات الخامات، توليد قيود التكاليف الصناعية التلقائية.',
      accountingImpact: 'قيد تكاليف صناعي آلي: (من حـ/ مخزون تام الصنع إلى حـ/ مخزون المواد الخام والمكونات + حـ/ تكاليف التشغيل الصناعية).',
      quickLinks: [
        { label: 'مركز التصنيع والإنتاج', tab: 'production' },
        { label: 'مستودع المنتجات التامة', tab: 'inventory' },
        { label: 'إدارة وتحديث الأسعار والتكاليف', tab: 'inventory' },
      ],
    },
    {
      id: 'step-4',
  ...
  return (
    <div ref={barRef} className="relative mb-3 no-print">
      {/* Top Header Row of the Bar */}
      <div className="bg-white border border-slate-200 rounded-xl p-2.5 shadow-2xs">
        <div className="flex items-center justify-between gap-2 px-1">
          <div className="flex items-center gap-2 min-w-0">
            <div className="p-1 bg-blue-50 text-blue-700 rounded-lg border border-blue-200/60 shrink-0">
              <Workflow className="w-3.5 h-3.5" />
            </div>
            <div className="min-w-0">
              <div className="flex items-center gap-1.5">
                <h3 className="text-xs font-bold text-slate-900 truncate">
                  دورة المحاسبة والتشغيل المتكاملة
                </h3>
                <span className="hidden sm:inline-block text-[10px] text-slate-500 font-medium truncate">
                  • {company?.nameAr || 'لوجيكس'}
                </span>
                <span className="text-[9px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 px-1.5 py-0.2 rounded-full shrink-0">
                  ترابط آلي
                </span>
              </div>
            </div>
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            ...
            <button
              ...
              className="px-2 py-1 bg-slate-800 hover:bg-slate-700 text-white rounded-lg text-xs font-bold flex items-center gap-1 shadow-2xs transition-all cursor-pointer"
            >
              <Sparkles className="w-3 h-3 text-cyan-300" />
              <span className="hidden md:inline">خريطة الترابط</span>
            </button>

            {/* Collapse/Expand Toggle */}
            <button
              onClick={() => setIsCollapsed(!isCollapsed)}
              title={isCollapsed ? 'إظهار مراحل دورة العمليات' : 'طي مراحل دورة العمليات لتوفير المساحة'}
              className="px-2 py-1 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg text-xs font-medium flex items-center gap-1 transition-all cursor-pointer"
            >
              <span>{isCollapsed ? 'عرض المراحل' : 'طي'}</span>
            </button>
          </div>
        </div>
        ...
        {!isCollapsed && (
          <div className="overflow-x-auto scrollbar-none pt-2 mt-2 border-t border-slate-100">
            <div className="flex items-center justify-between min-w-[920px] gap-1">
              {steps.map((step, idx) => {
                ...
                return (
                  <React.Fragment key={step.id}>
                    <div className="relative group">
                      <div
                        className={`flex items-center justify-between gap-1.5 px-2 py-1.5 rounded-lg transition-all border select-none cursor-pointer ${
                          isTabActive
                            ? 'bg-[#0F2942] text-white shadow-xs border-blue-500/50'
                      ...
                      >
                        ...
                        <div className="flex items-center gap-1.5">
                          ...
                          <div className="text-right">
                            ...
                            <div
                              className={`text-[9px] whitespace-nowrap truncate max-w-[95px] ${
