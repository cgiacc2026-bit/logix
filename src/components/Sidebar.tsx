...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <aside
      className={`fixed top-0 right-0 h-screen z-40 flex flex-col transition-all duration-300 no-print border-l border-slate-800 font-sans bg-[#0F172A] ${
        collapsed ? 'w-16' : 'w-64'
      }`}
    >
      {/* 1. Header Section */}
      <div className="h-[60px] flex items-center justify-between px-3 border-b border-slate-800/80 shrink-0 bg-[#0B1120]">
        {!collapsed ? (
        ...
        <button
          ...
          className="p-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-400 hover:text-white border border-slate-700 transition-colors cursor-pointer shrink-0"
        >
          {collapsed ? (
      ...
      <div className="flex-1 overflow-y-auto py-3 px-2 space-y-2 scrollbar-thin scrollbar-thumb-slate-800 scrollbar-track-transparent">
        ...
        {/* The 5 Collapsible Accordion Sections */}
        {filteredSections.map((section) => {
          ...
          return (
            ...
            >
              ...
              {isOpen && (
                <div className="px-2 pb-2.5 pt-0.5 space-y-1 pr-3 mr-3 border-r border-slate-700/60 transition-all animate-fadeIn">
                  ...
                  {section.reportItems.length > 0 && (
                    <div className="pt-2 mt-2 space-y-1">
                      ...
                      <div className="px-2 py-1 flex items-center justify-between border-t border-slate-800/80 text-[10px] text-slate-400 font-bold">
                        <span className="flex items-center gap-1.5 text-emerald-400">
                          <FileBarChart className="w-3 h-3 text-emerald-400" />
                          <span>تقارير القسم</span>
                        </span>
                        <span className="text-[9px] text-slate-500 font-mono">REPORTS</span>
                      </div>
      ...
      {/* 3. Footer Section */}
      {!collapsed && (
        <div className="p-3 border-t border-slate-800 bg-[#0B1120] text-[11px] text-slate-400 flex flex-col gap-1 shrink-0">
