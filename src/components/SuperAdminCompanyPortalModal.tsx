...
export const SuperAdminCompanyPortalModal: React.FC<SuperAdminCompanyPortalModalProps> = ({
  ...
  const [cloudUrl, setCloudUrl] = useState<string>(getSupabaseConfig().url || 'https://tshcwdieqlldkygkcytr.supabase.co');
  ...
  // Permanent Cascading Delete Modal state
  const [deleteTargetCompany, setDeleteTargetCompany] = useState<TenantCompanyRecord | null>(null);
  const [deleteStats, setDeleteStats] = useState<Record<string, number> | null>(null);
  const [isLoadingDeleteStats, setIsLoadingDeleteStats] = useState<boolean>(false);
  const [typeConfirmName, setTypeConfirmName] = useState<string>('');
  const [isAcknowledged, setIsAcknowledged] = useState<boolean>(false);
  const [isDeletingPermanently, setIsDeletingPermanently] = useState<boolean>(false);

  const openDeleteModal = async (comp: TenantCompanyRecord) => {
    setDeleteTargetCompany(comp);
    setTypeConfirmName('');
    ...
    try {
      const res = await getCompanyStatsForSuperAdmin(comp.id);
      if (res && res.counts) {
        setDeleteStats(res.counts);
      } else {
        setDeleteStats({});
      }
    } catch {
  ...
  const handleExecutePermanentDelete = async () => {
    ...
    if (typeConfirmName.trim() !== deleteTargetCompany.company_name.trim()) return;
    if (!isAcknowledged) return;

    setIsDeletingPermanently(true);
    try {
      const res = await deleteCompanyPermanently(
        deleteTargetCompany.id,
        typeConfirmName.trim(),
        currentUser?.email || 'Super Admin (cgiacc2026@gmail.com)'
      );

      if (res.success) {
        setActionMessage({
          type: 'success',
          text: `تم حذف شركة "${deleteTargetCompany.company_name}" وجميع بياناتها وسجلاتها نهائياً بنجاح! 🗑️`,
        });
        ...
        setActionMessage({
          type: 'error',
          text: res.message || 'فشل حذف المنشأة',
        });
      }
    } catch (err: any) {
      setActionMessage({
                          ...
                          <div className="flex items-center justify-center gap-1.5">
                            ...
                            {comp.status === 'suspended' && (
                              <>
                                ...
                                {comp.id !== '20000000-0000-0000-0000-000000000001' && comp.id !== '10000000-0000-0000-0000-000000000001' && (
                                  <button
                                    onClick={() => openDeleteModal(comp)}
                                    className="px-2.5 py-1 bg-rose-50 hover:bg-rose-100 text-rose-700 hover:text-rose-800 rounded-lg font-bold text-xs transition-all border border-rose-200 cursor-pointer flex items-center gap-1 shadow-xs"
                                    title="حذف نهائي وشامل للمنشأة وسجلاتها (Cascading Delete)"
                                  >
          ...
          <div className="bg-white rounded-2xl max-w-xl w-full p-6 space-y-4 shadow-2xl border border-slate-200">
            ...
            <div className="bg-blue-50/70 border border-blue-200 rounded-xl p-3.5 space-y-2 text-xs">
              ...
              <ol className="list-decimal list-inside space-y-1 text-[11px] text-blue-800 leading-relaxed">
                <li>افتح لوحة مشروعك في Supabase (المشروع: <strong className="font-mono text-slate-900">tshcwdieqlldkygkcytr</strong>).</li>
            ...
            <div className="space-y-3 text-xs">
              <div>
                ...
                <input
                  ...
                  placeholder="https://tshcwdieqlldkygkcytr.supabase.co"
...
CREATE INDEX IF NOT EXISTS idx_customers_name_search ON customers(company_id, name);`;
      ...
      {deleteTargetCompany && (
        <div className="fixed inset-0 z-70 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl max-w-xl w-full p-6 space-y-4 shadow-2xl border border-rose-200 overflow-hidden">
            {/* Modal Header */}
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-base font-black text-rose-900">
                    حذف نهائي وشامل للمنشأة (Cascading Delete)
                  </h3>
                  <p className="text-xs text-rose-600 font-semibold">
                    إجراء أمني حرج — حذف متسلسل دائم لا يمكن التراجع عنه
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="bg-rose-50 border border-rose-200 rounded-xl p-3 text-xs text-rose-800 space-y-1">
              <p className="font-bold flex items-center gap-1.5 text-rose-900">
                ...
                تأكيد مضاعف لحماية البيانات:
              </p>
              <p className="leading-relaxed">
                تم استيفاء شرط الأمان الأول (تعليق المنشأة). أنت الآن بصدد الخطوة الثانية: مسح المنشأة بالكامل وكافة السجلات المرتبطة بها في قاعدة البيانات السحابية Supabase بترتيب يحترم قيود المفاتيح الأجنبية، مع توثيق العملية في سجل التدقيق الدائم (Audit Logs).
              </p>
            </div>
            ...
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 text-xs space-y-1.5">
              <div className="flex justify-between">
                <span className="text-slate-500 font-bold">اسم المنشأة:</span>
                <span className="font-black text-slate-900">{deleteTargetCompany.company_name}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500 font-bold">البريد الإلكتروني:</span>
                <span className="font-mono text-slate-700">{deleteTargetCompany.owner_email}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500 font-bold">معرّف المنشأة (UUID):</span>
                <span className="font-mono text-[11px] text-slate-500">{deleteTargetCompany.id}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-slate-500 font-bold">الحالة الحالية:</span>
                <span className="px-2 py-0.5 rounded bg-rose-100 text-rose-800 font-bold text-[10px]">معلقة / مؤهلة للحذف</span>
              </div>
            </div>

            {/* Records Breakdown (Audit Pre-check) */}
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                <span className="text-xs font-black text-slate-700 flex items-center gap-1">
                  <Database className="w-3.5 h-3.5 text-indigo-600" />
                  حصر السجلات المرتبطة بالمنشأة المقرر حذفها:
                </span>
                {isLoadingDeleteStats && (
                  <span className="text-[11px] text-indigo-600 flex items-center gap-1 font-bold animate-pulse">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 text-[11px]">
                <div className="bg-slate-50 border border-slate-200 rounded-lg p-2 flex justify-between items-center">
                  ...
                  <span className="font-black font-mono text-slate-900">{deleteStats?.invoices ?? '...'}</span>
                </div>
                <div className="bg-slate-50 border border-slate-200 rounded-lg p-2 flex justify-between items-center">
                  <span className="text-slate-600">بنود الفواتير:</span>
                  <span className="font-black font-mono text-slate-900">{deleteStats?.invoice_items ?? '...'}</span>
                </div>
                <div className="bg-slate-50 border border-slate-200 rounded-lg p-2 flex justify-between items-center">
                  <span className="text-slate-600">قيود اليومية:</span>
                  <span className="font-black font-mono text-slate-900">{deleteStats?.journal_entries ?? '...'}</span>
                </div>
                <div className="bg-slate-50 border border-slate-200 rounded-lg p-2 flex justify-between items-center">
                  <span className="text-slate-600">أسطر القيود:</span>
                  <span className="font-black font-mono text-slate-900">{deleteStats?.journal_entry_lines ?? '...'}</span>
                </div>
                <div className="bg-slate-50 border border-slate-200 rounded-lg p-2 flex justify-between items-center">
                  <span className="text-slate-600">سندات القبض/الصرف:</span>
                  <span className="font-black font-mono text-slate-900">{deleteStats?.payment_vouchers ?? '...'}</span>
                </div>
                <div className="bg-slate-50 border border-slate-200 rounded-lg p-2 flex justify-between items-center">
                  <span className="text-slate-600">العملاء:</span>
                  <span className="font-black font-mono text-slate-900">{deleteStats?.customers ?? '...'}</span>
                </div>
                <div className="bg-slate-50 border border-slate-200 rounded-lg p-2 flex justify-between items-center">
                  <span className="text-slate-600">الموردين:</span>
                  <span className="font-black font-mono text-slate-900">{deleteStats?.suppliers ?? '...'}</span>
                </div>
                <div className="bg-slate-50 border border-slate-200 rounded-lg p-2 flex justify-between items-center">
                  <span className="text-slate-600">الأصناف:</span>
                  <span className="font-black font-mono text-slate-900">{deleteStats?.items ?? '...'}</span>
                </div>
                <div className="bg-slate-50 border border-slate-200 rounded-lg p-2 flex justify-between items-center">
                  <span className="text-slate-600">المستودعات:</span>
                  <span className="font-black font-mono text-slate-900">{deleteStats?.warehouses ?? '...'}</span>
                </div>
                <div className="bg-slate-50 border border-slate-200 rounded-lg p-2 flex justify-between items-center">
                  <span className="text-slate-600">دليل الحسابات:</span>
                  <span className="font-black font-mono text-slate-900">{deleteStats?.chart_of_accounts ?? '...'}</span>
                </div>
                <div className="bg-slate-50 border border-slate-200 rounded-lg p-2 flex justify-between items-center">
                  <span className="text-slate-600">إعدادات المحاسبة:</span>
                  <span className="font-black font-mono text-slate-900">{deleteStats?.company_accounting_settings ?? '...'}</span>
                </div>
                <div className="bg-rose-50 border border-rose-200 rounded-lg p-2 flex justify-between items-center font-bold">
                  <span className="text-rose-800">إجمالي السجلات:</span>
                  <span className="font-black font-mono text-rose-900">
                    {deleteStats ? Object.values(deleteStats).reduce((a, b) => a + b, 0) : '...'}
                  </span>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2 pt-2 border-t border-slate-100">
              <label className="block text-xs font-bold text-slate-700">
                لتأكيد الحذف النهائي، يرجى كتابة اسم المنشأة تماماً كالمطلوب:
              </label>
              <div className="p-2 bg-slate-100 rounded-lg font-mono font-black text-center text-xs text-slate-900 select-all border border-slate-200">
                {deleteTargetCompany.company_name}
              </div>
              <input
                type="text"
                value={typeConfirmName}
                onChange={(e) => setTypeConfirmName(e.target.value)}
                placeholder="أدخل اسم المنشأة هنا للمطابقة..."
                className="w-full bg-slate-50 border border-slate-300 focus:border-rose-500 rounded-xl p-2.5 text-xs font-bold outline-none focus:ring-2 focus:ring-rose-200 transition-all text-center"
                dir="auto"
                disabled={isDeletingPermanently}
              />
              ...
              <label className="flex items-start gap-2 pt-1 cursor-pointer select-none">
                ...
                <span className="text-[11px] text-slate-600 font-semibold leading-tight">
                  أقر بصفتي المشرف العام بأنني راجعت السجلات أعلاه وأوافق صراحة على الحذف النهائي الشامل (Cascading Delete) لشركة <strong className="text-slate-900">{deleteTargetCompany.company_name}</strong> دون إمكانية للاسترجاع.
                </span>
              </label>
            </div>
            ...
            <div className="flex items-center justify-end gap-2 pt-3 border-t border-slate-100">
              <button
                onClick={() => setDeleteTargetCompany(null)}
                disabled={isDeletingPermanently}
                className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl font-bold text-xs cursor-pointer transition-all disabled:opacity-40"
              >
                إلغاء الأمر
              </button>
              ...
              <button
                ...
                disabled={
                  typeConfirmName.trim() !== deleteTargetCompany.company_name.trim() ||
                  !isAcknowledged ||
                  isDeletingPermanently
                }
                className="px-5 py-2 bg-rose-600 hover:bg-rose-700 disabled:bg-slate-200 disabled:text-slate-400 text-white rounded-xl font-black text-xs flex items-center gap-1.5 cursor-pointer transition-all disabled:cursor-not-allowed shadow-md hover:shadow-lg shadow-rose-200"
              >
                {isDeletingPermanently ? (
                  <>
                    <RefreshCw className="w-3.5 h-3.5 animate-spin" />
                    <span>جارٍ الحذف المتسلسل والتوثيق...</span>
                  </>
                ) : (
                  <>
