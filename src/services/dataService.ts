...
export class DataService {
  ...
  public static reconcileDocumentCycles(): { linkedInvoices: number; linkedVouchers: number; generatedJournals: number } {
    ...
    if (localDataStore.isAlWaleedActive()) {
      ...
      // 1b. Ensure opening inventory valuation journal JV-2026-0002 (dated 2026-07-31)
      const invOpeningAmt = 4992.030;
      let obInvJournal = journals.find((j) => j.entryNumber === 'JV-2026-0002' || j.reference === 'OB-INV-2026-07-31');
      if (!obInvJournal) {
        obInvJournal = {
          id: 'jv-ob-inv-2026-07-31',
          companyId: '20000000-0000-0000-0000-000000000001',
          entryNumber: 'JV-2026-0002',
          date: '2026-07-31',
          reference: 'OB-INV-2026-07-31',
          description: 'بضاعة أول المدة وتقييم المخزون الافتتاحي بالمستودع بتاريخ 2026-07-31',
          status: 'POSTED',
          totalDebit: invOpeningAmt,
          totalCredit: invOpeningAmt,
          createdAt: '2026-07-31T00:00:00.000Z',
          postedAt: '2026-07-31T00:00:00.000Z',
          sourceModule: 'OPENING',
          isAutoGenerated: false,
          lines: [
            {
              ...
              debit: invOpeningAmt,
              credit: 0,
              memo: 'إجمالي تقييم بضاعة أول المدة بالمستودع',
            },
            {
              id: 'line-ob-inv-cr',
              accountId: 'acc-3100',
              ...
              accountNameAr: 'رأس المال المكتتب به / الأرصدة الافتتاحية',
              debit: 0,
              credit: invOpeningAmt,
              memo: 'مقابل رصيد بضاعة أول المدة',
            },
          ],
        };
        journals.splice(1, 0, obInvJournal);
        generatedJournals++;
      } else {
        if (obInvJournal.status !== 'POSTED') obInvJournal.status = 'POSTED';
        if (obInvJournal.date !== '2026-07-31') obInvJournal.date = '2026-07-31';
        if (Math.abs(Number(obInvJournal.totalDebit) - invOpeningAmt) > 0.01) {
          obInvJournal.totalDebit = invOpeningAmt;
          obInvJournal.totalCredit = invOpeningAmt;
          if (obInvJournal.lines?.[0]) obInvJournal.lines[0].debit = invOpeningAmt;
          if (obInvJournal.lines?.[1]) obInvJournal.lines[1].credit = invOpeningAmt;
        }
      }
      ...
      if (c4640 && (Number(c4640.openingBalance) === 0 || c4640.openingBalanceDate !== '2026-07-31')) {
        c4640.openingBalance = 4921.986;
